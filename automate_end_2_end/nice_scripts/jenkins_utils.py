...
def create_job(job_name,xml, found_job, build = False):
    try:
        if not found_job:
...
def build_job(job_name, repo_branch, slave):
    server.build_job(job_name, {'BRANCH_NAME': repo_branch,  'SLAVE':slave})


def code_startup(dict_parameters, found_job, build = False):
    if found_job:
    ...
    create_job(dict_parameters['job_name'], xml, found_job, build= build)
