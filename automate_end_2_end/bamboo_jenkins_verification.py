    ...
    for idx, job in jobs_on_jenkins.iterrows():
        while True:
            try:
                ...
                if not bamboo_plan.empty:
                    ...
                    if bamboo_artifacts and jenkins_artifacts:
                        for art in jenkins_artifacts:
                            print(art)
                            df = pd.DataFrame(columns = ['job_name','file_name', 'jenkins_job_url', 'is_msi','file_size_j','percent_error','Success','art_found_bamboo'])
                ...
                print("Got ConnectionError error ")
                attempt += 1
                continue
            break
...
if __name__ == '__main__':
    jobs_on_bamboo = pd.read_csv('jobs_on_bamboo.csv')
    jobs_on_jenkins = pd.read_csv('40 Jenkin jobs.csv')
    verify_artifacts(jobs_on_bamboo=jobs_on_bamboo, jobs_on_jenkins=jobs_on_jenkins)